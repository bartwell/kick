...
class TestDataInitializer(context: PlatformContext) {
    ...
    private val testLogs: Array<String> = arrayOf(
        "[Android][UI] In a world where Android apps learn from user behavior before they even realize what they want, your code becomes the real miracle worker.",
        "[Tests] Every test scenario is like an expedition into the unknown depths of your codebase, where hidden bugs await discovery around every corner.",
        "[Tests][CI] When a Kotlin-based robot orders a virtual espresso, make sure it has both the right permissions and the spark of creativity to enjoy it.",
        "[Compose][UI] Compose Multiplatform will soon unite all screens into a single harmonious symphony of interfaces—are you ready for the concerto?",
        "[Tests][CI] While you sleep, the CI server runs endless test suites so that your library remains flawless and ever-prepared for battle.",
        "[Tests] Your suite of unit tests should be as unbreakable as the legendary code of a mythic hero in an epic saga.",
        "[Logging] Sometimes a bug is just a glimpse into the future, nudging you toward a revolutionary optimization you hadn’t yet imagined.",
        "[Logging] A robust logging system records not only runtime errors but also those fleeting moments of inspiration that strike at 3 AM.",
        ...
        "[CI] Your next pull request could revolutionize the project’s architecture, as long as you never forget the principles of clean code and friendly tests."
    )
