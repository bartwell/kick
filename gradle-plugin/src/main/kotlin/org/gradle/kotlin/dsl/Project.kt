 ...
 * - [enabled] = true → use full runtime (equivalent to kick { enabled() })
 * - [enabled] = false → use stub (equivalent to kick { disabled() })
 *
 ...
 *
 * Priority: CLI -Pkick.enabled > enableKick() > kick { enabledAuto() / enabled() / disabled() }
 */
